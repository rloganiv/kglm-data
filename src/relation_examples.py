...
def main(_):
    ...
    for instance in generate_instances(FLAGS.input):
        ...
        for annotation in instance['annotations']:
            relation, qualifier = annotation['relation']
            # Use human readable names for relations and unique ids.
            _, relation = relation
            _, qualifier = qualifier
            id, name = annotation['id']
            parent_id, parent_name = annotation['parent_id']
            span = annotation['span']
            source = annotation['source']

            # If relation is interesting get the span of text
            if relation:
                relation_counts[(relation, qualifier)] += 1
                if parent_id in last_seen:
                    ...
                    too_long = (end - start) > FLAGS.l if FLAGS.l else False
                    if not too_long:
                        ...
                        examples[(relation, qualifier)].append(excerpt)
