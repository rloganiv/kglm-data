...
def _bad_alias(tokens):
    if all(x.tag_ in PRONOUNS for x in tokens):
        return True
    if all(x.pos_ == 'PUNCT' for x in tokens):
        return True
    if all(x.text in STOP_WORDS for x in tokens):
        return True
    if all(x.text.lower() in STOP_WORDS for x in tokens):
        return True
    return False
...
class Annotator:
        ...
        for start, end in cluster:
            ...
            for token, type in zip(mention_tokens, mention_types):
                ...
                if not_in_alias and not_det:
                    logger.debug('Encountered invalid token: "%s" - rejecting', token)
                    keep = False
                    break
            if keep:
                new_cluster.append([start, end])
    ...
    def _propagate_ids(self, doc: Doc,
        ...
        for i, cluster in enumerate(clusters):
            ...
            if self._prune_clusters:
                ...
                logger.debug('Detected following prepruned ids for cluster %i: %s', i, pre_pruned_cluster_ids)
                if len(pre_pruned_cluster_ids) != 1:
                    logger.debug('Number of prepruned ids is not equal to 1, skipping propagation')
                    continue
                ...
                for alias in aliases:
                    if _bad_alias(self._nlp.tokenizer(alias)):
                        continue
                    alias_tokens = tuple(x.text for x in self._nlp.tokenizer(alias))
                    alias_token_set.update(alias_tokens)
                    alias_token_set.update(capitalize(alias_tokens))
                logger.debug('Alias token set: %s', alias_token_set)
    ...
    def _add_aliases(self, id: str) -> None:
        ...
        try:
            ...
            for alias in aliases:
                ...
                self._alias_lookup.add(capitalize(alias_tokens), id)
        ...
        for prop, value in self._relation_db[id]:
            ...
            if prop == 'P395':
                ...
                for child_alias in child_aliases:
                    if _bad_alias(self._nlp.tokenizer(child_alias)):
                        continue
                    ...
                    if child_alias_tokens not in self._alias_lookup:
                        ...
                        self._alias_lookup.add(capitalize(child_alias_tokens), id)
                return
            ...
            for child_alias in child_aliases:
                child_alias_tokens = [x.text for x in self._nlp.tokenizer(child_alias)]
                logger.debug('Child alias tokens %s', child_alias_tokens)
                if _bad_alias(self._nlp.tokenizer(child_alias)):
                    logger.debug('Rejected')
                    continue
                if child_alias_tokens not in self._alias_lookup:
                    self._alias_lookup.add(child_alias_tokens, child_id)
                    self._alias_lookup.add(capitalize(child_alias_tokens), child_id)
    ...
    def _annotate_tokens(self, doc: Doc) -> Doc:
        ...
        while token_stack:
            ...
            if len(relation) == 0:
                logger.debug('No surviving story')
                if RE_ENTITY.match(id) and any(token._.source is not None for token, _ in match_stack):
                    relation = ['@@NEW@@']
                    parent_id = [id]
                else:
                    logger.debug('Not an entity, rejecting')
